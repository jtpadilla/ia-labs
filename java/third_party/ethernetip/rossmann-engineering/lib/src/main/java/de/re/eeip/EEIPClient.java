...
public class EEIPClient
{
    ...
    public static long ToUint(byte[] byteArray)
    {
        ...
        /**
         * Calcula la dirección multicast CIP (según Vol.2, 3-5.3.2)
         * a partir de la dirección IP del dispositivo.
         *
         * @param deviceIPAddress IPv4 empaquetada en int (big endian)
         * @return dirección multicast como int (interpretar como uint32)
         */
        private static int GetMulticastAddress(int deviceIPAddress)
        {
            // Base CIP multicast: 239.192.1.0
            final long cip_Mcast_Base_Addr = 0xEFC00100L;
            final int cip_Host_Mask = 0x3FF; // 10 bits

            // Tratar la IP como valor no signado de 32 bits
            long addr = deviceIPAddress & 0xFFFFFFFFL;

            int netmask;

            // Clasificación por clase, usando máscaras de bits sobre addr (unsigned)
            // Clase A: 0xxxxxxx ........
            ...
            // Clase B: 10xxxxxx ........
            else if ((addr & 0xC0000000L) == 0x80000000L)
            {
                netmask = 0xFFFF0000;
            }
            // Clase C: 110xxxxx ........
            else if ((addr & 0xE0000000L) == 0xC0000000L)
            {
                netmask = 0xFFFFFF00;
            }
            else
            {
                // Fuera de A/B/C → no hay fórmula definida; aquí
                // devolvemos simplemente la base (o podrías lanzar excepción).
                return (int)cip_Mcast_Base_Addr;
            }

            long netmaskUnsigned = netmask & 0xFFFFFFFFL;

            // Host ID dentro de la red
            long hostID = addr & ~netmaskUnsigned;

            // Índice multicast (hostID empieza en 1)
            long mcastIndex = (hostID - 1) & cip_Host_Mask;

            long multicastAddress = cip_Mcast_Base_Addr + (mcastIndex * 32L);

            // Devuelto como int, pero representa un uint32 (IPv4)
            return (int)multicastAddress;
        }
